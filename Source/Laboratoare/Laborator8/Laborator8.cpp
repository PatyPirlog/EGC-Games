...
void Laborator8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
	...
	int type = glGetUniformLocation(shader->program, "spot");
...
void Laborator8::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		// pt bonus
		if (window->KeyHold(GLFW_KEY_U)) angleX += deltaTime * 5;
		if (window->KeyHold(GLFW_KEY_H)) angleY += deltaTime * 5;
		if (window->KeyHold(GLFW_KEY_J)) angleX -= deltaTime * 5;
		if (window->KeyHold(GLFW_KEY_K)) angleY -= deltaTime * 5;
		if (window->KeyHold(GLFW_KEY_O)) cutoff_angle += deltaTime * 20;
		if (window->KeyHold(GLFW_KEY_P)) cutoff_angle -= deltaTime * 20;

		glm::mat4 miscare = glm::mat4(1);
		miscare = glm::rotate(miscare, angleY, glm::vec3(0, 1, 0));
		miscare = glm::rotate(miscare, angleX, glm::vec3(1, 0, 0));

		lightDirection = glm::vec3(0, -1, 0);
		lightDirection = glm::vec3(miscare * glm::vec4(lightDirection, 0));
	}
}
